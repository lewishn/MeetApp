<?php
	...
	if(isset($_POST['email']) && isset($_POST['password']) && isset($_POST['name']))
	{
		...
		$result = pg_query($db,"SELECT uid FROM users WHERE email='$email'");
		$count = pg_num_rows($result);
		$row = pg_fetch_array($result, PGSQL_ASSOC);
		// If result matched $username and $password, table row  must be 1 row
		if($count == 0)
		{
			$addquery = pg_query($db, "INSERT INTO users (name,email,pwd) VALUES('$name','$email','$pwd')");
			$result = pg_query($db,"SELECT uid,name FROM users WHERE email='$email'");
			$row = pg_fetch_array($result, PGSQL_ASSOC);
